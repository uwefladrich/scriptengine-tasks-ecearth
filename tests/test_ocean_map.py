"""Tests for scriptengine/tasks/ecearth/monitoring/ocean_map.py"""

import iris

from scriptengine.tasks.ecearth.monitoring.ocean_map import OceanMap
...
def test_ocean_map_once(tmpdir):
    ...
    ocean_map = OceanMap(init)
    ocean_map.run(init)
    cube = iris.load_cube(init['dst'])
    assert cube.attributes['map_type'] == ocean_map.map_type
    assert cube.attributes['diagnostic_type'] == ocean_map.diagnostic_type
