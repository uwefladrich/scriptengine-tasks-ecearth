...
def get_connection_details():
    """Provide URL, KEY for Redmine server"""

    url = 'https://dev.ec-earth.org'
    key = '0eeb96bc61b6fdf2de480596877106cf2325a0a4'
...
class RedmineOutput(Task):
    ...
    def run(self, context):
        ...
        self.log_debug(f"Template: {template}, Source File(s): {sources}, Local storage: {dst_folder}")
        ...
        redmine_template = environment.get_template(template)

        with ChangeDirectory(dst_folder):
            with open("./description.txt", 'w') as outfile:
                outfile.write(redmine_template.render(
                    presentation_list=presentation_list,
                ))
            with open("./description.txt", 'r') as outfile:
                description = outfile.readlines()
        
        url, key = get_connection_details()
    ...
    def get_issue(self, redmine, issue_subject):
        ...
        #     error('Could not connect to Redmine Server (ConnectionError)')
